...
import storyChaptersData from '@/data/mock/our-story.json'
import rsvpInfosData from '@/data/mock/info.json'
...
const mockDataMap: Record<string, any> = {
  ...
  'our-story.json': storyChaptersData,
  'info.json': rsvpInfosData,
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetStoryChapters') || query.includes('nodeStoryChapters')) {
      return loadMockData('our-story.json')
    }

    if (query.includes('GetRsvpInfos') || query.includes('nodeRsvpInfos')) {
      return loadMockData('info.json')
    }
