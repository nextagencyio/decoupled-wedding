...
import { GET_RSVP_INFO_BY_PATH } from '@/lib/queries'
...
interface RsvpInfoByPathData {
  route: {
    entity: DrupalRsvpInfo
  } | null
}
...
async function getRsvpInfo(path: string): Promise<DrupalRsvpInfo | null> {
  try {
    ...
    const { data } = await apolloClient.query<RsvpInfoByPathData>({
      query: GET_RSVP_INFO_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/info/${slug.join('/')}`
  const item = await getRsvpInfo(path)

  if (!item) {
    return { title: 'Rsvp Info Not Found | Wedding' }
  }
...
export default async function RsvpInfoDetailPage({ params }: PageProps) {
  const { slug } = await params
  const path = `/info/${slug.join('/')}`
  const item = await getRsvpInfo(path)
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).infoCategory && (
                    <div>
                      <dt className="text-sm text-gray-500">Info Category</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).infoCategory}</dd>
                    </div>
                  )}
                  {(item as any).contactInfo && (
                    <div>
                      <dt className="text-sm text-gray-500">Contact Info</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).contactInfo}</dd>
                    </div>
                  )}
                  {(item as any).deadline && (
                    <div>
                      <dt className="text-sm text-gray-500">Deadline</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).deadline}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
