...
interface StoryChapterCardProps {
  item: DrupalStoryChapter
}

export default function StoryChapterCard({ item }: StoryChapterCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      ...
      <div className="p-6">
          {(item as any).chapterDate && (
            <p className="text-sm text-zinc-700 font-medium mb-2">{(item as any).chapterDate}</p>
          )}
